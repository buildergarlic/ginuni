...
| 검사 | 결과 |
| --- | --- |
| 전체 단위 테스트 | 235개 통과, 30개 파일 |
| TypeScript / 제품 빌드 | 통과 |
| 새 작업 화면 자동 검사 | 8개 시나리오 통과 |
| 기존 검증 가능한 AI 워크플로우 | 8개 시나리오 통과 |
| 유튜브 제어 경계 검사 | 4개 통과 |
| 실제 유튜브 영상 클릭 이동 | 4개 통과 |
| 사용법 / About / Threads | 5개 통과 |
| 렌더러 미처리 오류 | 위 실행에서 0개 |
| 시각 검증 | 2회 비교 후 P0/P1/P2 미해결 항목 없음 |

새 화면 검사는 초안 저장 후 승인, 잘못된 시간 입력, 빠른 두 번 클릭, 저장 실패, 모달 키보드 이동, 미저장 동의 선택 유지, 실제 HWPX/SRT 산출물, 1440/1100/800px 화면, 긴 시간값, 큰 한글, 고대비를 포함한다. 기존 검사는 교정 제안 적용 후 원문 보존, 저장본 복구, 내보낸 뒤 계속 편집하기를 포함한다.
