...
다음은 공개판과 검증 완료로 안내하는 릴리스의 기본 절차입니다. 제한된 파일럿 베타는 아래 예외 원칙에 따라 사람 검증 일부를 배포 뒤에 진행할 수 있습니다.
...
6. `CHANGELOG.md`와 `package.json` 버전을 갱신하고, 해당 태그 이름으로 `docs/release-notes-vX.Y.Z.md`를 작성합니다. GitHub 릴리스 본문에는 이 파일이 자동으로 첨부됩니다.
...
소유자가 제한된 참여자를 위한 파일럿 베타 배포를 명시적으로 승인하면, 아래 사람 검증 항목을 아직 수행 중인 상태에서도 `0.x` 시험판을 배포할 수 있습니다. 이 승인은 파일럿을 시작할 권한일 뿐, 실제 작가 사용성·정확도·납품 적합성·공개판 기준 통과를 뜻하지 않습니다.

- 릴리스 노트와 다운로드 안내에 `파일럿 베타`임과 아직 완료하지 않은 사람 검증을 적습니다.
- 자동 테스트·합성 자료·개발자 점검과 실제 작가 파일럿 결과를 구분합니다.
- 태그 전 `npm ci`, `npm run verify`, 설치본 생성은 생략하지 않습니다. 한컴오피스 실문서·실제 작가 등 뒤에 진행할 사람 검증은 릴리스 노트에 구체적으로 적습니다. 게시 후에는 GitHub Actions 성공, 설치본·블록맵·`latest.yml`·SHA-256, 실제 서명 상태를 다시 확인합니다.
- 기존 베타 설치본에서 새 버전을 찾고 내려받는지 확인하되, 자동 업데이트 확인만으로 설치·데이터 보존 검증을 대신하지 않습니다.
- 아래 파일럿 통과 기준을 모두 충족하기 전에는 `1.0.0`, 공개판, 검증 완료 또는 납품 적합으로 표현하지 않습니다.
...
`v0.6.0-beta.1` 파일럿 시작 승인 시점에는 위 기준 외에도 실제 화면해설작가 5–8명의 첫 사용·오류 복구·납품 동선, 실제 OpenAI 계정 연결, 한컴오피스 긴 문서 페이지 나눔·서식 검증이 남아 있습니다. 이 결과는 파일럿 후 별도로 기록하며 사후에 통과한 것으로 소급 표현하지 않습니다.
