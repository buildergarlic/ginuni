    ...
    <div className="manual-shell">
      ...
      <section className="manual-section" aria-labelledby="guide-start">
        ...
        <ol className="manual-steps">
          ...
          <li><span className="manual-number">3</span><h3>듣고, 쓰고, 저장해요</h3><p>영상을 보며 대사를 고치고, 해설 칸에는 작가님이 직접 화면해설을 작성하세요.</p><p>확인한 행은 <b>선택한 행 확인 완료</b>로 표시한 뒤 <b>HWPX 내보내기</b>로 저장합니다.</p><span className="manual-caption">문장을 고치는 것과 확인 완료는 별개예요</span></li>
        </ol>
      ...
      <section className="manual-section" aria-labelledby="guide-check">
        ...
        <ol className="manual-check-flow"><li><strong>다음 확인할 행</strong><span>남은 곳으로 이동</span></li><li><strong>영상과 대본 대조</strong><span>대사·해설·시간 확인</span></li><li><strong>선택한 행 확인 완료</strong><span>내가 확인했다는 표시</span></li></ol>
        <p>확인 완료한 행도 다시 수정하면 확인 전으로 돌아갑니다. 실수가 아니라 <b>수정한 내용을 한 번 더 살펴보라는 표시</b>예요.</p>
        <div className="manual-pair manual-exports"><article><span className="manual-file">HWPX</span><h3>화면해설 대본으로 저장</h3><p><b>HWPX 내보내기</b> → 저장 폴더 선택.<br />대사와 작가가 쓴 해설을 문서로 꺼냅니다.</p><p>한컴오피스에서 문장을 읽고, 긴 문서의 페이지 나눔과 서식을 확인하세요.</p></article><article><span className="manual-file">SRT</span><h3>대사 자막으로 저장</h3><p><b>SRT 내보내기</b> → 저장 폴더 선택.<br /><b>대사 행만</b> 들어갑니다. 해설 행은 포함되지 않습니다.</p><p>화면해설 대본이 필요하다면 HWPX를 선택하세요.</p></article></div>
        ...
        <p>시간·내용 오류는 먼저 고쳐야 저장할 수 있습니다. 확인 전 행이 남았다면 검수를 계속하거나, 경고를 확인한 뒤 현재 내용을 작업용 파일로 저장할 수 있습니다. 작업용 파일을 최종 검수본으로 취급하지 마세요.</p>
      </section>
      ...
      <section className="manual-section" aria-labelledby="guide-extras">
        ...
        <details className="manual-detail"><summary>선택 사항 · AI에게 대사 교정 제안 받기 <span className="manual-cost">유료 · 외부 전송</span></summary><div><ol><li><b>설정</b>에서 OpenAI API 키를 저장합니다. 키는 비밀번호처럼 다른 사람에게 공유하지 마세요.</li><li><b>사용 권리와 외부 전송 설정</b>에서 선택한 대사 텍스트 전송에 동의하고 <b>동의 설정 저장</b>을 누릅니다. 음성 전송 동의와는 별개입니다.</li><li>대사 한 행을 선택한 뒤 <b>선택한 대사 교정 요청 (유료)</b>을 누릅니다.</li><li>수정 전·제안·이유를 원음과 비교하고 <b>제안 적용</b> 또는 <b>제안 거절</b>을 선택합니다.</li></ol><p>선택한 대사 텍스트가 OpenAI로 전송되고 API 사용료가 발생합니다. AI가 뜻을 바꿀 수 있으므로 그대로 믿지 마세요. 적용 후에는 다시 확인해야 합니다.</p></div></details>
        <details className="manual-detail"><summary>이전 대본으로 되돌리고 싶어요</summary><div><p><b>이전 상태로 복구 → 이전 저장 목록 보기 → 복구할 저장본 선택 → 선택한 저장본으로 복구</b> 순서로 누르세요.</p><p>주요 작업 전 남겨 둔 최근 10개 복구본을 제공합니다. 모든 타이핑 순간이 남는 것은 아닙니다. 복구 직전 내용도 저장본으로 남지만, 영상 파일까지 백업하는 기능은 아닙니다.</p></div></details>
        <details className="manual-detail"><summary>목소리 구분·다른 음성 분석 방식을 쓰고 싶어요</summary><div><p>새 대본 만들기의 <b>고급 옵션 · 음성 분석 방식 변경</b>에서 선택합니다. 처음에는 기본값을 유지해도 됩니다.</p><ul><li><b>내 PC에서 분석:</b> 대사 초안을 만듭니다. 음성 외부 전송과 API 사용료가 없습니다.</li><li><b>내 PC + 화자 분리:</b> 목소리를 구분하는 실험 기능입니다. 잘못 나뉜 화자는 직접 고치세요.</li><li><b>OpenAI로 분석:</b> API 키와 음성 외부 전송 동의가 필요하며 API 사용료가 발생합니다.</li></ul></div></details>
        <details className="manual-detail"><summary>어디를 고쳤는지 확인하고 싶어요</summary><div><p><b>선택한 행의 원래 음성 인식 결과</b>에서 초안을 비교하고, <b>작업 기록 · 고급 정보</b>에서 분석·수정·확인 기록을 살펴볼 수 있습니다.</p><p><b>작업 기록 파일 저장</b>으로 꺼낸 기록에는 대본 내용이 포함될 수 있습니다. 문의용 <b>진단 파일</b>과는 다르니 외부에 공유하기 전에 내용을 확인하세요.</p></div></details>
      </section>
      ...
      <section className="manual-section" aria-labelledby="guide-help">
        ...
        <details className="manual-detail"><summary>저장 버튼을 누를 수 없어요</summary><div><p><b>시간 오류</b> 또는 <b>자동 점검 알림</b>을 펼치고 해당 행으로 이동하세요. 시작·종료 역전, 시간 겹침, 빈 내용 등을 수정합니다. 입력 중인 시간 값도 끝까지 완성해 주세요.</p><p>확인 전 행과 시간 오류는 달라요. 확인 전 행은 작업용 저장 경고가 나오지만, 잘못된 시간은 먼저 수정해야 합니다.</p></div></details>
        ...
        <details className="manual-detail"><summary>예전 대본이 모두 확인 전으로 표시돼요</summary><div><p>이전 버전의 확인 기록을 새 방식으로 옮기면서 다시 확인하도록 표시할 수 있습니다. 대사와 해설이 사라진 것은 아닙니다. 원본 영상과 대조한 뒤 <b>선택한 행 확인 완료</b>로 표시하세요.</p></div></details>
        <details className="manual-detail"><summary>어디에 저장되나요? 업데이트는 어떻게 하나요?</summary><div><p>프로젝트는 Windows 문서 폴더의 <b>화면해설 대본 도구 → Projects</b>에 저장됩니다. 정확한 위치는 앱의 <b>설정</b>에서 확인하세요. 로컬 원본 영상은 작업 중 이동하거나 삭제하지 마세요.</p><p><b>About GiNuNi → 업데이트 확인</b>으로 새 버전을 확인합니다. 내려받기가 끝나면 <b>재시작하여 업데이트</b>를 누르세요. 설치 전 프로젝트와 원본 영상을 별도로 백업하는 것이 좋습니다.</p><p>무서명 파일럿은 Windows 경고가 나타날 수 있습니다. ABOUT의 GitHub 저장소에서 공식 Releases 출처를 확인하세요.</p></div></details>
      </section>
