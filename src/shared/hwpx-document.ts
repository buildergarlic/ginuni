// Shared by the desktop and browser exports so both preserve the same HWPX layout.
...
import { formatTimecode, formatIntervalSeconds } from './timecode'
...
/** Transform template contents without filesystem, Node Buffer, or archive dependencies. */
export function renderHwpxEntries(template: ReadonlyMap<string, Uint8Array>, title: string, rows: ScriptRow[]): Map<string, Uint8Array> {
  const decoder = new TextDecoder()
  const encoder = new TextEncoder()
  const sectionBytes = template.get(SECTION_PATH)
  const contentBytes = template.get(CONTENT_PATH)
  if (!sectionBytes?.length || !contentBytes?.length) throw new Error('HWPX 템플릿 핵심 XML이 없습니다.')

  const entries = new Map(template)
  entries.set(SECTION_PATH, encoder.encode(updateSection(decoder.decode(sectionBytes), title, rows)))
  entries.set(CONTENT_PATH, encoder.encode(updateContentMetadata(decoder.decode(contentBytes), title)))
  entries.set(PREVIEW_TEXT_PATH, encoder.encode(previewText(title, rows)))
  entries.delete(PREVIEW_IMAGE_PATH)
  entries.set('mimetype', encoder.encode('application/hwp+zip'))
  return entries
}
